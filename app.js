...
let phGpsWatchId=null,phLastGpsPoint=null,phGpsDistanceMiles=0,phManualDistance=false,phTimerPaused=false,phTimerInitialSeconds=0;
function phDistanceMiles(a,b){const R=3958.7613,toRad=x=>x*Math.PI/180,dLat=toRad(b.lat-a.lat),dLon=toRad(b.lon-a.lon),q=Math.sin(dLat/2)**2+Math.cos(toRad(a.lat))*Math.cos(toRad(b.lat))*Math.sin(dLon/2)**2;return 2*R*Math.asin(Math.sqrt(q))}
function phStopGps(){if(phGpsWatchId!==null&&navigator.geolocation){navigator.geolocation.clearWatch(phGpsWatchId);phGpsWatchId=null}phLastGpsPoint=null}
function phStartGps(){phStopGps();phGpsDistanceMiles=0;phManualDistance=false;const status=document.getElementById('gpsLiveStatus');if(!navigator.geolocation){if(status){status.textContent='GPS is unavailable in this browser. Use manual distance if needed.';status.className='gps-status gps-error'}return}if(status){status.textContent='Requesting precise location…';status.className='gps-status'}phGpsWatchId=navigator.geolocation.watchPosition(pos=>{if(!currentActivity)return;const p={lat:pos.coords.latitude,lon:pos.coords.longitude,accuracy:pos.coords.accuracy,time:pos.timestamp};if(p.accuracy>80){if(status)status.textContent=`GPS signal is weak (±${Math.round(p.accuracy)} m). Keep moving in an open area.`;return}if(phLastGpsPoint){const segment=phDistanceMiles(phLastGpsPoint,p);const seconds=Math.max(1,(p.time-phLastGpsPoint.time)/1000);const mph=segment/(seconds/3600);if(segment<0.2&&mph<15)phGpsDistanceMiles+=segment}phLastGpsPoint=p;if(!phManualDistance)updateActivityDistance(phGpsDistanceMiles,false);if(status){status.textContent=`GPS active • accuracy ±${Math.round(p.accuracy)} m`;status.className='gps-status gps-live'}},err=>{if(status){status.textContent=`GPS unavailable: ${err.message}. You can enter distance manually.`;status.className='gps-status gps-error'}},{enableHighAccuracy:true,maximumAge:2000,timeout:15000})}
const phOriginalStartActivity=startActivity;
startActivity=function(){phOriginalStartActivity();if(currentActivity&&['walk','run','bike'].includes(currentActivity.type))phStartGps();else{const s=document.getElementById('gpsLiveStatus');if(s)s.textContent='GPS is not needed for this activity.'}}
const phOriginalPauseActivity=pauseActivity;
pauseActivity=function(){phOriginalPauseActivity();if(currentActivity?.running&&['walk','run','bike'].includes(currentActivity.type))phStartGps();else phStopGps()}
const phOriginalCloseActivity=closeActivity;
closeActivity=function(){phStopGps();phOriginalCloseActivity()}
const phOriginalFinishActivity=finishActivity;
finishActivity=function(){phStopGps();phOriginalFinishActivity()}
updateActivityDistance=function(v,manual=false){if(!currentActivity)return;if(manual)phManualDistance=true;currentActivity.distance=Math.max(0,Number(v||0));if(window.activityDistance)activityDistance.textContent=currentActivity.distance.toFixed(2);let mins=currentActivity.elapsed/60;if(currentActivity.distance>0&&mins>0){const pace=mins/currentActivity.distance;activityPace.textContent=pace<100?pace.toFixed(1):'—'}if(!manual&&window.activityDistanceInput)activityDistanceInput.value=currentActivity.distance?currentActivity.distance.toFixed(2):''}

// Persistent timer with pause/reset and top-of-app placement
const phOldStartTimer=startTimer,phOldStartPhaseTimer=startPhaseTimer;
startTimer=function(seconds){phTimerInitialSeconds=seconds;phTimerPaused=false;if(window.timerPauseBtn)timerPauseBtn.textContent='Pause';phOldStartTimer(seconds)}
startPhaseTimer=function(minutes,label){phTimerInitialSeconds=minutes*60;phTimerPaused=false;if(window.timerPauseBtn)timerPauseBtn.textContent='Pause';phOldStartPhaseTimer(minutes,label)}
function toggleTimerPause(){if(!timerHandle&&timerSeconds>0){phTimerPaused=false;timerHandle=setInterval(()=>{timerSeconds--;drawTimer();if(timerSeconds<=0){stopTimer();if(navigator.vibrate)navigator.vibrate([180,80,180])}},1000);if(window.timerPauseBtn)timerPauseBtn.textContent='Pause';return}if(timerHandle){clearInterval(timerHandle);timerHandle=null;phTimerPaused=true;if(window.timerPauseBtn)timerPauseBtn.textContent='Resume'}}
function resetActiveTimer(){if(timerHandle)clearInterval(timerHandle);timerHandle=null;timerSeconds=phTimerInitialSeconds||60;drawTimer();restTimer.classList.add('show');phTimerPaused=true;if(window.timerPauseBtn)timerPauseBtn.textContent='Start'}

// Itemized nutrition estimation database. Values are common estimates per listed serving.
const PH_FOODS={
 'egg':{serving:'large egg',cal:72,p:6.3,c:.4,f:4.8},'eggs':{serving:'large egg',cal:72,p:6.3,c:.4,f:4.8},'toast':{serving:'slice',cal:80,p:3,c:14,f:1},'bread':{serving:'slice',cal:80,p:3,c:14,f:1},'oatmeal':{serving:'cup cooked',cal:154,p:6,c:27,f:3},'banana':{serving:'medium',cal:105,p:1.3,c:27,f:.4},'apple':{serving:'medium',cal:95,p:.5,c:25,f:.3},'orange':{serving:'medium',cal:62,p:1.2,c:15,f:.2},'rice':{serving:'cup cooked',cal:205,p:4.3,c:45,f:.4},'brown rice':{serving:'cup cooked',cal:216,p:5,c:45,f:1.8},'chicken breast':{serving:'4 oz cooked',cal:187,p:35,c:0,f:4},'chicken':{serving:'4 oz cooked',cal:220,p:30,c:0,f:10},'ground beef':{serving:'4 oz cooked',cal:287,p:26,c:0,f:20},'steak':{serving:'4 oz cooked',cal:250,p:28,c:0,f:15},'salmon':{serving:'4 oz cooked',cal:233,p:25,c:0,f:14},'tuna':{serving:'4 oz',cal:132,p:29,c:0,f:1},'tilapia':{serving:'4 oz cooked',cal:145,p:30,c:0,f:3},'potato':{serving:'medium',cal:161,p:4,c:37,f:.2},'sweet potato':{serving:'medium',cal:112,p:2,c:26,f:.1},'broccoli':{serving:'cup',cal:55,p:3.7,c:11,f:.6},'mixed vegetables':{serving:'cup',cal:80,p:4,c:15,f:1},'salad':{serving:'2 cups',cal:40,p:2,c:8,f:.5},'avocado':{serving:'half',cal:120,p:1.5,c:6,f:11},'cheese':{serving:'1 oz',cal:115,p:7,c:1,f:9},'greek yogurt':{serving:'cup',cal:130,p:20,c:9,f:0},'yogurt':{serving:'cup',cal:150,p:9,c:17,f:4},'milk':{serving:'cup',cal:122,p:8,c:12,f:4.8},'protein shake':{serving:'shake',cal:180,p:30,c:8,f:3},'protein powder':{serving:'scoop',cal:120,p:24,c:3,f:2},'peanut butter':{serving:'2 tbsp',cal:190,p:7,c:7,f:16},'tortilla':{serving:'medium',cal:140,p:4,c:24,f:4},'beans':{serving:'cup',cal:225,p:15,c:40,f:1},'pasta':{serving:'cup cooked',cal:220,p:8,c:43,f:1.3},'pizza':{serving:'slice',cal:285,p:12,c:36,f:10},'burger':{serving:'burger',cal:520,p:28,c:40,f:28},'fries':{serving:'medium',cal:365,p:4,c:48,f:17},'bacon':{serving:'2 slices',cal:87,p:6,c:.3,f:7},'coffee':{serving:'black cup',cal:2,p:.3,c:0,f:0},'soda':{serving:'12 oz',cal:150,p:0,c:39,f:0},'juice':{serving:'8 oz',cal:110,p:1,c:26,f:0},'sports drink':{serving:'20 oz',cal:130,p:0,c:34,f:0},'beer':{serving:'12 oz',cal:150,p:1.6,c:13,f:0},'water':{serving:'cup',cal:0,p:0,c:0,f:0}
};
let phMealRows=[];
function phFoodMatch(name){const n=(name||'').trim().toLowerCase();if(!n)return null;return PH_FOODS[n]||Object.entries(PH_FOODS).sort((a,b)=>b[0].length-a[0].length).find(([k])=>n.includes(k))?.[1]||null}
function phNewMealRow(kind='food',seed={}){return{id:seed.id||crypto.randomUUID(),kind,name:seed.name||'',qty:Number(seed.qty||1),unit:seed.unit||'',calories:seed.calories??'',protein:seed.protein??'',carbs:seed.carbs??'',fat:seed.fat??''}}
function addFoodRow(kind='food',seed={}){phMealRows.push(phNewMealRow(kind,seed));renderFoodRows()}
function removeFoodRow(id){phMealRows=phMealRows.filter(x=>x.id!==id);if(!phMealRows.some(x=>x.kind==='food'))phMealRows.push(phNewMealRow('food'));renderFoodRows()}
function updateFoodRow(id,key,value){const r=phMealRows.find(x=>x.id===id);if(!r)return;r[key]=key==='qty'?Math.max(.01,Number(value||1)):value;if(key==='name'){const match=phFoodMatch(value);if(match){r.unit=match.serving;r.calories=match.cal;r.protein=match.p;r.carbs=match.c;r.fat=match.f}}renderFoodTotals();if(key==='name')renderFoodRows()}
function renderFoodRows(){for(const kind of ['food','drink']){const el=document.getElementById(kind==='food'?'mealFoodItems':'mealDrinkItems');if(!el)continue;const rows=phMealRows.filter(x=>x.kind===kind);el.innerHTML=rows.map(r=>`<div class="food-item-row"><input class="food-name" value="${String(r.name).replace(/"/g,'&quot;')}" placeholder="${kind==='food'?'Chicken breast, rice, banana…':'Coffee, soda, protein shake…'}" onchange="updateFoodRow('${r.id}','name',this.value)"><input type="number" min=".01" step=".25" value="${r.qty}" onchange="updateFoodRow('${r.id}','qty',this.value)" aria-label="Quantity"><input value="${String(r.unit||'').replace(/"/g,'&quot;')}" placeholder="serving" onchange="updateFoodRow('${r.id}','unit',this.value)" aria-label="Serving"><input type="number" min="0" value="${r.calories}" placeholder="cal" onchange="updateFoodRow('${r.id}','calories',this.value)" aria-label="Calories"><button class="danger" onclick="removeFoodRow('${r.id}')">×</button></div>`).join('')||`<div class="muted small">No ${kind}s added.</div>`}renderFoodTotals()}
function phMealTotals(rows=phMealRows){return rows.reduce((t,r)=>{const q=Number(r.qty||1);t.calories+=Number(r.calories||0)*q;t.protein+=Number(r.protein||0)*q;t.carbs+=Number(r.carbs||0)*q;t.fat+=Number(r.fat||0)*q;return t},{calories:0,protein:0,carbs:0,fat:0})}
function renderFoodTotals(){const t=phMealTotals();if(window.mealCaloriesTotal)mealCaloriesTotal.textContent=Math.round(t.calories);if(window.mealProteinTotal)mealProteinTotal.textContent=Math.round(t.protein)+'g';if(window.mealCarbsTotal)mealCarbsTotal.textContent=Math.round(t.carbs)+'g';if(window.mealFatTotal)mealFatTotal.textContent=Math.round(t.fat)+'g'}
openMeal=function(){editingMealId=null;mealModalTitle.textContent='Add Meal or Snack';mealHunger.value='';mealFullness.value='';mealPlan.value='About as planned';phMealRows=[phNewMealRow('food'),phNewMealRow('food'),phNewMealRow('drink')];renderFoodRows();mealModal.classList.add('show')}
editMeal=function(id){let m=state.meals.find(x=>x.id===id);if(!m)return;editingMealId=id;mealModalTitle.textContent='Edit Entry';mealType.value=m.type;mealHunger.value=m.hunger||'';mealFullness.value=m.fullness||'';mealPlan.value=m.plan||'About as planned';phMealRows=(m.items&&m.items.length?m.items:[phNewMealRow('food',{name:m.description||'',qty:1,calories:m.calories||''})]).map(x=>phNewMealRow(x.kind||'food',x));if(!phMealRows.some(x=>x.kind==='drink'))phMealRows.push(phNewMealRow('drink'));renderFoodRows();mealModal.classList.add('show')}
saveMeal=function(){const rows=phMealRows.filter(r=>r.name.trim());if(!rows.length)return alert('Add at least one food or drink.');const date=foodOffset===0?today():foodDateKey(),tot=phMealTotals(rows),desc=rows.map(r=>`${r.qty} ${r.unit||'serving'} ${r.name}`).join(', '),item={id:editingMealId||crypto.randomUUID(),date,type:mealType.value,description:desc,items:rows,hunger:mealHunger.value,fullness:mealFullness.value,plan:mealPlan.value,time:new Date().toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}),calories:Math.round(tot.calories),protein:Math.round(tot.protein),carbs:Math.round(tot.carbs),fat:Math.round(tot.fat),estimate:true};if(editingMealId)state.meals=state.meals.map(m=>m.id===editingMealId?{...m,...item}:m);else state.meals.push(item);save();closeMeal();renderTodayMeals();renderFood();updateMomentum();foodCoach.textContent=item.plan.includes('more')||item.plan.includes('Unplanned')?'Thanks for logging it honestly. Keep the next meal normal and improve one part—drink, portion, or side.':'Good. Repeat what worked without trying to make the whole day perfect.'}
renderMealsInto=function(el,k,compact=false){let list=state.meals.filter(m=>m.date===k).sort((a,b)=>(a.time||'').localeCompare(b.time||''));if(!list.length){el.innerHTML='<p class="muted">No meals or snacks logged yet.</p>';return}const total=list.reduce((a,m)=>a+Number(m.calories||0),0);el.innerHTML=`${compact?'':`<div class="nutrition-total" style="margin-bottom:10px"><strong>${Math.round(total)}</strong><span class="muted small">estimated calories logged</span></div>`}`+list.map(m=>{const items=(m.items||[]).filter(x=>x.name);return `<div class="meal-card"><div class="top" style="margin:0"><div><strong>${m.type}</strong><div class="muted small">${m.time||''}</div></div><span class="pill">${m.plan||'Logged'}</span></div>${items.length?`<ul class="meal-items-list">${items.map(x=>`<li>${x.qty||1} ${x.unit||'serving'} ${x.name} — ${Math.round(Number(x.calories||0)*Number(x.qty||1))} cal</li>`).join('')}</ul>`:`<div>${m.description||''}</div>`}<div class="meal-calories">${Number.isFinite(Number(m.calories))?Math.round(Number(m.calories))+' estimated calories':'Calories not estimated'}${m.protein?` • ${m.protein}g protein`:''}</div>${compact?'':`<div class="muted small">Hunger: ${m.hunger||'—'} • Fullness: ${m.fullness||'—'}</div><div class="meal-actions"><button class="secondary" onclick="editMeal('${m.id}')">Edit</button><button class="danger" onclick="deleteMeal('${m.id}')">Delete</button></div>`}</div>`}).join('')}

// Treadmill phase controls, calorie estimates, and reversible completion.
function phStrengthCalories(minutes,met=5.5){return Math.max(1,Math.round(met*3.5*profileWeightKg()/200*minutes))}
function phTreadmillCalories(minutes,speed,incline){let met=Number(speed)>=5?8.3:Number(incline)>=6?6.0:Number(speed)>=3.5?4.8:3.5;met+=Math.min(2,Number(incline||0)*.12);return Math.round(met*3.5*profileWeightKg()/200*minutes)}
function phToggleGuidedPhase(){const item=guidedItems[guidedIndex];item.done=!item.done;renderGuidedStep()}
markGuidedDone=function(){phToggleGuidedPhase()}
markPhaseDone=function(btn,type){const done=btn.dataset.done!=='true';btn.dataset.done=String(done);btn.textContent=done?'Undo Complete':'Mark Complete';btn.classList.toggle('undo-complete',done);btn.closest('.phase-card').dataset.phase=type}
const phOldRenderGuidedStep=renderGuidedStep;
renderGuidedStep=function(){phOldRenderGuidedStep();const item=guidedItems[guidedIndex];if(!item)return;if(item.type==='warmup'||item.type==='finisher'){const card=exerciseList.querySelector('.phase-card');if(!card)return;const p=item.phase||{},defaultIncline=parseFloat(String(p.incline||'4').match(/[\d.]+/)?.[0]||4),defaultSpeed=parseFloat(String(p.speed||'3.2').match(/[\d.]+/)?.[0]||3.2);card.querySelector('.muted').textContent='Set the treadmill to your actual values below.';const controls=document.createElement('div');controls.className='phase-metrics';controls.innerHTML=`<label>Speed (mph)<input id="phTreadmillSpeed" type="number" min="0" max="15" step=".1" value="${defaultSpeed}"></label><label>Incline (%)<input id="phTreadmillIncline" type="number" min="0" max="30" step=".5" value="${defaultIncline}"></label>`;const pEl=card.querySelector('p');pEl.after(controls);const estimate=document.createElement('div');estimate.className='suggestion exercise-calorie';estimate.id='phTreadmillEstimate';const refresh=()=>estimate.innerHTML=`<strong>Estimated burn:</strong> ${phTreadmillCalories(item.minutes,Number(phTreadmillSpeed.value),Number(phTreadmillIncline.value))} calories for ${item.minutes} minutes.`;controls.after(estimate);controls.querySelectorAll('input').forEach(x=>x.addEventListener('input',refresh));refresh();const buttons=card.querySelectorAll('button');const doneBtn=buttons[buttons.length-1];doneBtn.textContent=item.done?'Undo Complete':'Mark Complete';doneBtn.classList.toggle('undo-complete',!!item.done);doneBtn.onclick=phToggleGuidedPhase;return}if(item.type==='exercise'){const e=item.exercise,minutes=Math.max(4,Number(e.sets||3)*2.5);const body=exerciseList.querySelector('.exercise-body');if(body){const est=document.createElement('div');est.className='pill exercise-calorie';est.textContent=`Estimated exercise burn: ${phStrengthCalories(minutes,Number(e.met||5.5))} calories`;body.querySelector('h1').after(est)}}}

// Add missing personal inputs without deleting existing profile fields.
function phInjectProfileVitals(){if(document.getElementById('profileAge'))return;const goal=document.getElementById('profileGoal');if(!goal)return;const wrap=document.createElement('div');wrap.className='profile-vitals';wrap.innerHTML='<div><label>Age</label><input id="profileAge" type="number" min="13" max="100"></div><div><label>Current weight (lb)</label><input id="profileWeight" type="number" min="60" max="700" step=".1"></div><div><label>Height (inches)</label><input id="profileHeight" type="number" min="36" max="96" step=".5"></div><div><label>Goal weight (lb)</label><input id="profileGoalWeight" type="number" min="60" max="700" step=".1"></div>';goal.closest('div')?.appendChild(wrap);profileAge.value=state.profile.age||'';profileWeight.value=state.profile.weight||'';profileHeight.value=state.profile.height||'';profileGoalWeight.value=state.profile.goalWeight||''}
const phOldInit=init;init=function(){phOldInit();setTimeout(phInjectProfileVitals,0)}
const phOldSaveProfile=typeof saveProfile==='function'?saveProfile:null;if(phOldSaveProfile){saveProfile=function(){state.profile.age=Number(document.getElementById('profileAge')?.value||state.profile.age||0)||'';state.profile.weight=Number(document.getElementById('profileWeight')?.value||state.profile.weight||0)||'';state.profile.height=Number(document.getElementById('profileHeight')?.value||state.profile.height||0)||'';state.profile.goalWeight=Number(document.getElementById('profileGoalWeight')?.value||state.profile.goalWeight||0)||'';phOldSaveProfile()}}


// ===== Coach PH Beta 1.0: inclusive journeys, founding testers, and plan generator =====
const BETA_VERSION='Coach PH Beta 1.0';
const BETA_JOURNEYS=[
 {id:'general',icon:'❤️',name:'General wellness',desc:'Energy, habits and consistency'},
 {id:'weight',icon:'⚖️',name:'Weight management',desc:'Sustainable daily choices'},
 {id:'walking',icon:'🚶',name:'Walking',desc:'Daily movement and walking plans'},
 {id:'running',icon:'🏃',name:'Running / jogging',desc:'Beginner through distance plans'},
 {id:'strength',icon:'🏋️',name:'Strength',desc:'Gym or home resistance training'},
 {id:'mobility',icon:'🤸',name:'Stretching & mobility',desc:'Flexibility and movement quality'},
 {id:'yoga',icon:'🧘',name:'Yoga',desc:'Strength, mobility and calm'},
 {id:'meditation',icon:'🧠',name:'Meditation',desc:'Focus, breathing and stress'},
 {id:'sleep',icon:'🌙',name:'Better sleep',desc:'Wind-down and consistency'},
 {id:'nutrition',icon:'🥗',name:'Nutrition',desc:'Meals, hydration and planning'},
 {id:'cycling',icon:'🚴',name:'Cycling',desc:'Indoor or outdoor riding'},
 {id:'recovery',icon:'🌿',name:'Recovery',desc:'Gentle movement and reset'}
];
const BETA_ACTIVITIES=[
 ['walking','Easy Walk','Cardio','None','Beginner','Comfortable walk; stay tall and use a natural arm swing.'],['walking','Brisk Walk','Cardio','None','Beginner','Walk at a pace that raises breathing while conversation remains possible.'],['walking','Walk Intervals','Cardio','None','Beginner','Alternate brisk and easy walking intervals.'],['walking','Long Easy Walk','Cardio','None','All levels','Build duration gradually at an easy effort.'],['running','Walk–Jog Intervals','Cardio','None','Beginner','Alternate short jogs with walking recovery.'],['running','Easy Jog','Cardio','None','Beginner','Keep effort conversational and stop before form breaks down.'],['running','Tempo Run','Cardio','None','Intermediate','Sustained comfortably-hard running after a warm-up.'],['running','Run Intervals','Cardio','None','Intermediate','Repeat short faster efforts with full controlled recovery.'],['strength','Conventional Deadlift','Posterior chain','Barbell','Intermediate','Brace, hinge, keep the bar close, and stand by driving through the floor.'],['strength','Romanian Deadlift','Hamstrings / glutes','Barbell or dumbbells','Intermediate','Push hips back with soft knees and a neutral spine.'],['strength','Sumo Deadlift','Glutes / legs','Barbell','Intermediate','Use a wider stance, brace, and keep knees tracking over toes.'],['strength','Trap-Bar Deadlift','Full body','Trap bar','Beginner','Brace and push the floor away while keeping the handles centered.'],['strength','Back Squat','Legs / core','Barbell and rack','Intermediate','Brace, sit between the hips, and keep feet rooted.'],['strength','Front Squat','Quads / core','Barbell and rack','Intermediate','Keep elbows high and torso upright through the squat.'],['strength','Goblet Squat','Legs / core','Dumbbell','Beginner','Hold weight at chest and squat between the hips.'],['strength','Box Squat','Legs / hips','Box and optional weight','Beginner','Control down to the box without collapsing, then stand strongly.'],['strength','Bench Press','Chest / triceps','Barbell and bench','Intermediate','Set shoulders, lower with control, and press over the shoulder line.'],['strength','Incline Dumbbell Press','Upper chest','Dumbbells and bench','Beginner','Press with wrists stacked and shoulders controlled.'],['strength','Overhead Press','Shoulders / triceps','Barbell or dumbbells','Intermediate','Brace ribs down and press overhead without leaning back.'],['strength','Barbell Row','Back / biceps','Barbell','Intermediate','Hinge, brace, and pull toward the lower ribs.'],['strength','One-Arm Dumbbell Row','Back / biceps','Dumbbell','Beginner','Keep hips square and pull elbow toward the back pocket.'],['strength','Lat Pulldown','Back / biceps','Cable machine','Beginner','Pull elbows down without swinging or leaning far back.'],['strength','Pull-Up','Back / arms','Pull-up bar','Intermediate','Start controlled and pull chest toward the bar.'],['strength','Hip Thrust','Glutes','Bench and weight','Beginner','Finish with ribs down and full hip extension.'],['strength','Split Squat','Legs / balance','Bodyweight or dumbbells','Beginner','Lower straight down with both feet planted.'],['strength','Walking Lunge','Legs / balance','Bodyweight or dumbbells','Intermediate','Step long enough to control the front knee.'],['strength','Push-Up','Chest / core','None','Beginner','Maintain a straight line and lower under control.'],['strength','Farmer Carry','Grip / core','Dumbbells or kettlebells','Beginner','Walk tall with controlled steps and steady breathing.'],['mobility','Morning Mobility Flow','Full body','None','Beginner','Gentle circles, reaches, hinges and controlled squats.'],['mobility','Hip Flexor Stretch','Hips','Mat','Beginner','Tuck pelvis gently and shift forward without low-back arching.'],['mobility','Hamstring Stretch','Hamstrings','None','Beginner','Hinge from the hips and avoid forcing range.'],['mobility','Calf Stretch','Calves / ankles','Wall','Beginner','Keep heel down and knee aligned over toes.'],['mobility','Thoracic Rotation','Upper back','Mat','Beginner','Rotate through the upper back while hips remain controlled.'],['mobility','Ankle Mobility','Ankles','Wall','Beginner','Drive knee forward over toes while keeping heel grounded.'],['mobility','Shoulder Mobility Flow','Shoulders','Band optional','Beginner','Move slowly through pain-free range.'],['mobility','Full-Body Cooldown','Full body','Mat','Beginner','Slow breathing with gentle stretches for trained muscles.'],['yoga','Beginner Yoga Flow','Full body','Mat','Beginner','Move slowly between stable positions and never force range.'],['yoga','Sun Salutation','Full body','Mat','Beginner','Link comfortable movement with steady breathing.'],['yoga','Restorative Yoga','Recovery','Mat / pillows','Beginner','Use supported poses and long relaxed breathing.'],['meditation','Box Breathing','Breathing','None','Beginner','Inhale, hold, exhale, and hold for equal comfortable counts.'],['meditation','5-Minute Mindfulness','Mental wellness','None','Beginner','Notice breath and sensations, returning gently when attention wanders.'],['meditation','Body Scan','Recovery / awareness','None','Beginner','Move attention through the body without judging sensations.'],['meditation','Evening Wind-Down','Sleep / stress','None','Beginner','Use slow exhalations and a quiet body scan.'],['recovery','Recovery Walk','Recovery','None','Beginner','Easy pace intended to feel better afterward, not tired.'],['recovery','Desk Reset','Mobility','Chair / none','Beginner','Short neck, chest, hip and ankle movements for workdays.'],['recovery','Foam-Rolling Basics','Recovery','Foam roller','Beginner','Use tolerable pressure and avoid rolling directly over joints.'],['cycling','Easy Ride','Cardio','Bicycle','Beginner','Comfortable cadence and easy breathing.'],['cycling','Cycling Intervals','Cardio','Bicycle','Intermediate','Alternate moderate-hard efforts with easy spinning.'],['nutrition','Meal Planning Session','Nutrition','None','All levels','Plan simple meals, protein sources, produce and hydration.'],['sleep','Sleep Wind-Down','Sleep','None','All levels','Dim stimulation, prepare tomorrow, and follow a calming routine.']
].map((x,i)=>({id:'beta_'+i,journey:x[0],name:x[1],focus:x[2],equipment:x[3],difficulty:x[4],instructions:x[5]}));
let betaObStep=0,betaSelected=new Set(),betaFilter='all',betaSubmitFile=null;
function betaProfile(){state.profile.coachBeta=state.profile.coachBeta||{};return state.profile.coachBeta}
function renderBetaJourneyChoices(){const el=document.getElementById('betaJourneyChoices');if(!el)return;el.innerHTML=BETA_JOURNEYS.map(j=>`<button type="button" class="beta-journey ${betaSelected.has(j.id)?'selected':''}" onclick="toggleBetaJourney('${j.id}',this)"><strong>${j.icon} ${j.name}</strong><span>${j.desc}</span></button>`).join('')}
function toggleBetaJourney(id,btn){betaSelected.has(id)?betaSelected.delete(id):betaSelected.add(id);btn.classList.toggle('selected',betaSelected.has(id))}
function openBetaOnboarding(reset=false){const p=betaProfile();betaSelected=new Set(reset?[]:(p.journeys||[]));betaObStep=0;renderBetaJourneyChoices();['Age','Weight','Height','GoalWeight'].forEach(k=>{const el=document.getElementById('beta'+k);if(el)el.value=p[k[0].toLowerCase()+k.slice(1)]||state.profile[k[0].toLowerCase()+k.slice(1)]||''});document.getElementById('betaOutcome').value=p.outcome||'';document.getElementById('betaExperience').value=p.experience||'New / returning';document.getElementById('betaDays').value=p.days||3;document.getElementById('betaMinutes').value=p.minutes||30;document.getElementById('betaPlace').value=p.place||'Anywhere / no equipment';document.getElementById('betaLimits').value=p.limits||state.profile.limitations||'';document.getElementById('betaFoodStyle').value=p.foodStyle||'No specific preference';document.getElementById('betaFoodAvoid').value=p.foodAvoid||'';renderBetaOb();document.getElementById('betaOnboardingModal').classList.add('show')}
function renderBetaOb(){document.querySelectorAll('.beta-step').forEach((x,i)=>x.classList.toggle('active',i===betaObStep));document.querySelectorAll('#betaDots i').forEach((x,i)=>x.classList.toggle('on',i<=betaObStep));document.getElementById('betaBack').style.visibility=betaObStep?'visible':'hidden';document.getElementById('betaNext').textContent=betaObStep===4?'Save and start':'Continue';if(betaObStep===4)renderBetaPlanPreview()}
function betaObBack(){if(betaObStep>0){betaObStep--;renderBetaOb()}}
function betaObNext(){if(betaObStep===0&&!betaSelected.size)return alert('Choose at least one health journey.');if(betaObStep<4){betaObStep++;renderBetaOb();return}saveBetaOnboarding()}
function betaPlanItems(journeys,days,mins){let items=[];const has=x=>journeys.includes(x);if(has('walking'))items.push(`A ${mins}-minute progressive walking session`);if(has('running'))items.push(`A walk–jog or running session matched to experience`);if(has('strength'))items.push(`A full-body strength session using available equipment`);if(has('mobility')||has('yoga'))items.push(`A mobility, stretching, or yoga session`);if(has('meditation')||has('sleep'))items.push(`A 5–10 minute breathing or wind-down practice`);if(has('cycling'))items.push(`An easy or interval cycling session`);if(has('nutrition')||has('weight'))items.push(`A nutrition planning and hydration check-in`);if(!items.length)items.push(`A ${mins}-minute general wellness session`);return items.slice(0,Math.max(2,Math.min(days,5)))}
function renderBetaPlanPreview(){const days=Number(betaDays.value||3),mins=Number(betaMinutes.value||30),journeys=[...betaSelected],items=betaPlanItems(journeys,days,mins);betaPlanPreview.innerHTML=`<div class="notice"><strong>${days}-day flexible plan</strong><div class="muted small">Coach PH will adapt the exact activity each day using sleep, energy, soreness, time and location.</div></div>${items.map((x,i)=>`<div class="beta-plan-day"><strong>Day ${i+1}</strong><span>${x}</span></div>`).join('')}<div class="beta-plan-day"><strong>Other days</strong><span>Recovery, easy movement, or rest based on readiness</span></div>`}
function saveBetaOnboarding(){const p=betaProfile();Object.assign(p,{onboarded:true,journeys:[...betaSelected],age:Number(betaAge.value)||'',weight:Number(betaWeight.value)||'',height:Number(betaHeight.value)||'',goalWeight:Number(betaGoalWeight.value)||'',outcome:betaOutcome.value.trim(),experience:betaExperience.value,days:Number(betaDays.value),minutes:Number(betaMinutes.value),place:betaPlace.value,limits:betaLimits.value.trim(),foodStyle:betaFoodStyle.value,foodAvoid:betaFoodAvoid.value.trim(),createdAt:p.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()});Object.assign(state.profile,{age:p.age||state.profile.age,weight:p.weight||state.profile.weight,height:p.height||state.profile.height,goalWeight:p.goalWeight||state.profile.goalWeight,goal:p.outcome||state.profile.goal,limitations:p.limits||state.profile.limitations,onboarded:true});save();betaOnboardingModal.classList.remove('show');renderBetaHub();toast('Coach PH plan saved')}
function betaJourneyName(id){return BETA_JOURNEYS.find(j=>j.id===id)?.name||id}
function betaWins(){const p=betaProfile(),j=p.journeys||['general'],wins=[];if(j.includes('walking')||j.includes('running'))wins.push(`Complete ${Math.min(30,p.minutes||30)} minutes of intentional movement`);else if(j.includes('strength'))wins.push('Complete today’s Coach PH activity');else wins.push('Move intentionally for at least 10 minutes');if(j.includes('nutrition')||j.includes('weight'))wins.push('Log one complete meal item by item');else wins.push('Drink water with your next meal');if(j.includes('meditation')||j.includes('sleep'))wins.push('Complete 5 minutes of quiet breathing');else if(j.includes('mobility')||j.includes('yoga'))wins.push('Complete a short mobility reset');else wins.push('Complete the evening reflection');return wins.slice(0,3)}
function toggleBetaWin(i,checked){const k=today(),p=betaProfile();p.wins=p.wins||{};p.wins[k]=p.wins[k]||[];p.wins[k][i]=checked;save()}
function renderBetaHub(){let host=document.getElementById('coachBetaHub');if(!host){host=document.createElement('div');host.id='coachBetaHub';const sec=document.getElementById('programs')||document.getElementById('today');sec?.prepend(host)}if(!host)return;const p=betaProfile();if(!p.onboarded){host.innerHTML=`<div class="founder-banner"><div class="eyebrow">COACH PH BETA 1</div><h2>Your health does not have to mean “going to the gym.”</h2><p class="muted">Choose walking, running, strength, stretching, yoga, meditation, sleep, nutrition, recovery—or combine them.</p><button onclick="openBetaOnboarding(true)">Build my personal health plan</button></div>`;return}const wins=betaWins(),done=p.wins?.[today()]||[];host.innerHTML=`<div class="founder-banner"><div class="eyebrow">YOUR ADAPTIVE HEALTH JOURNEY</div><h2>${(p.journeys||[]).map(betaJourneyName).join(' • ')}</h2><p class="muted">${p.outcome||'Build a healthier life one decision at a time.'}</p><div class="grid"><button onclick="openBetaOnboarding()">Update my plan</button><button class="secondary" onclick="openBetaLibrary()">Explore all activities</button></div></div><div class="card"><div class="top"><div><div class="eyebrow">WIN TODAY</div><h2>Three achievable wins</h2></div><span class="pill">${done.filter(Boolean).length}/3</span></div>${wins.map((w,i)=>`<label class="beta-win"><input type="checkbox" ${done[i]?'checked':''} onchange="toggleBetaWin(${i},this.checked);renderBetaHub()"><span><strong>${w}</strong><div class="muted small">Coach PH values consistency over perfection.</div></span></label>`).join('')}</div><div class="card"><div class="top"><div><div class="eyebrow">FOUNDING TESTER</div><h2>Help build the activity library</h2></div></div><p class="muted">Exercise cards without final media invite testers to submit an original image or short video for review.</p><button onclick="openBetaLibrary()">Open activity library</button></div>`}
function betaIcon(j){return ({walking:'🚶',running:'🏃',strength:'🏋️',mobility:'🤸',yoga:'🧘',meditation:'🧠',recovery:'🌿',cycling:'🚴',nutrition:'🥗',sleep:'🌙'})[j]||'❤️'}
function openBetaLibrary(){betaFilter='all';let detail=document.getElementById('betaActivityDetail');detail.innerHTML=`<div class="eyebrow">ACTIVITY LIBRARY</div><h1>Health is bigger than the gym</h1><div class="beta-filter" id="betaFilters"></div><input id="betaLibrarySearch" placeholder="Search activities, muscles, equipment…" oninput="renderBetaLibrary()"><div id="betaLibraryGrid" class="beta-library" style="margin-top:14px"></div>`;betaActivityModal.classList.add('show');renderBetaFilters();renderBetaLibrary()}
function renderBetaFilters(){const cats=['all',...new Set(BETA_ACTIVITIES.map(x=>x.journey))];betaFilters.innerHTML=cats.map(c=>`<button class="${betaFilter===c?'active':'secondary'}" onclick="betaFilter='${c}';renderBetaFilters();renderBetaLibrary()">${c==='all'?'All':betaJourneyName(c)}</button>`).join('')}
function renderBetaLibrary(){const q=(document.getElementById('betaLibrarySearch')?.value||'').toLowerCase();const rows=BETA_ACTIVITIES.filter(a=>(betaFilter==='all'||a.journey===betaFilter)&&(`${a.name} ${a.focus} ${a.equipment}`).toLowerCase().includes(q));betaLibraryGrid.innerHTML=rows.map(a=>`<div class="beta-activity-card"><div class="beta-media"><div><b>${betaIcon(a.journey)}</b><span>SUBMIT YOUR BEST IMAGE<br>FOR COACH PH</span></div></div><div class="beta-activity-body"><h3>${a.name}</h3><div class="beta-tags">${a.focus} • ${a.equipment}<br>${a.difficulty}</div><button class="secondary" onclick="showBetaActivity('${a.id}')">View</button></div></div>`).join('')}
function showBetaActivity(id){const a=BETA_ACTIVITIES.find(x=>x.id===id);if(!a)return;betaActivityDetail.innerHTML=`<div class="beta-media" style="border-radius:20px"><div><b>${betaIcon(a.journey)}</b><span>SUBMIT YOUR BEST IMAGE OR VIDEO<br>FOR POSSIBLE USE</span></div></div><div class="eyebrow" style="margin-top:16px">${betaJourneyName(a.journey)}</div><h1>${a.name}</h1><div class="grid"><div class="notice"><strong>Focus</strong><div>${a.focus}</div></div><div class="notice"><strong>Equipment</strong><div>${a.equipment}</div></div></div><h3>How to perform it</h3><p>${a.instructions}</p><div class="notice">Start conservatively. Use pain-free range and follow any professional restrictions.</div><button style="margin-top:12px" onclick="openBetaSubmission('${a.name.replace(/'/g,"\\'")}')">Submit an image or video</button><button class="secondary" style="margin-top:8px" onclick="openBetaLibrary()">Back to library</button>`}
function closeBetaActivity(){betaActivityModal.classList.remove('show')}
function openBetaSubmission(name){betaSubmissionActivity.value=name;betaSubmissionNote.value='';betaSubmissionFile.value='';betaSubmissionRights.checked=false;betaSubmissionPreview.innerHTML='';betaSubmitFile=null;betaSubmitModal.classList.add('show')}
function closeBetaSubmission(){betaSubmitModal.classList.remove('show')}
function previewBetaSubmission(){const f=betaSubmissionFile.files?.[0];betaSubmitFile=f||null;if(!f)return betaSubmissionPreview.innerHTML='';if(f.size>25*1024*1024){betaSubmissionFile.value='';betaSubmitFile=null;return alert('Please choose a file under 25 MB for the beta.')}const u=URL.createObjectURL(f);betaSubmissionPreview.innerHTML=f.type.startsWith('video')?`<video class="submission-preview" controls src="${u}"></video>`:`<img class="submission-preview" src="${u}">`}
function saveBetaSubmission(){if(!betaSubmitFile)return alert('Choose a photo or video.');if(!betaSubmissionRights.checked)return alert('Confirm that you have rights to submit this media.');state.betaMediaSubmissions=state.betaMediaSubmissions||[];state.betaMediaSubmissions.push({id:crypto.randomUUID(),activity:betaSubmissionActivity.value,note:betaSubmissionNote.value.trim(),fileName:betaSubmitFile.name,fileType:betaSubmitFile.type,fileSize:betaSubmitFile.size,status:'pending-local-export',createdAt:new Date().toISOString(),tester:state.profile.name||'Tester',version:BETA_VERSION});save();closeBetaSubmission();toast('Submission record queued. Export the tester package to share it.')}
function betaInjectFeedbackCategories(){const s=document.getElementById('feedbackCategory');if(!s||[...s.options].some(o=>o.value==='Activity media submission'))return;['Activity media submission','Onboarding and plan','Walking / running GPS','Meditation or stretching','Founding tester idea'].forEach(x=>s.add(new Option(x,x)))}
const betaPriorInit=init;init=function(){betaPriorInit();setTimeout(()=>{renderBetaHub();betaInjectFeedbackCategories();const p=betaProfile();if(!p.onboarded)setTimeout(()=>openBetaOnboarding(true),700)},50)}
