...
async function boot(){ensureProfiles();try{WORKOUTS=await fetch("data/workouts.json?build=122",{cache:"no-store"}).then(r=>r.json())}catch(e){alert("Workout data could not load.");return}init();bindProgramControls();initTesterBeta();window.ProjectHealthCloud?.initialize?.();if(!state.profile.onboarded)setTimeout(()=>onboardingModal.classList.add("show"),250)}
