...
function switchProfile(id){if(!profileDB.profiles[id])return;activeProfileId=id;state=profileDB.profiles[id].state;state.activities=state.activities||[];save();init();renderWeek();renderFood();renderProgress()}
...
function renderWorkoutModal(){
 let w=WORKOUTS[dayKey(selectedDate)],plan=activeAdaptivePlan||buildAdaptivePlan({date:selectedDate,time:activeMode==="full"?60:activeMode==="short"?30:15,effort:"moderate"});
