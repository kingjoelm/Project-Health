...
async function boot(){ensureProfiles();try{WORKOUTS=await fetch("data/workouts.json?build=121",{cache:"no-store"}).then(r=>r.json())}catch(e){alert("Workout data could not load.");return}init();bindProgramControls();initTesterBeta();window.ProjectHealthCloud?.initialize?.();if(!state.profile.onboarded)setTimeout(()=>onboardingModal.classList.add("show"),250)}
...
if("serviceWorker"in navigator)navigator.serviceWorker.register("service-worker.js?build=121").then(r=>r.update()).catch(()=>{});boot();